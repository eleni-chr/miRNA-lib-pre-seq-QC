function plotTapeStationPredominantPeakHistogram(excelFile)
...
% with electronic ladder (EL) that represent the predominant peak (i.e.,
% the peak with the largest integrated area percentage) among different library
% samples. The function assumes that the peak sizes are stored in the fifth column and the peak 
% sizes with EL are in the sixth column of the Excel file, both expressed in 
...
%              fifth column and the peak sizes with EL in the sixth column, 
%              both in base pairs (bp).
%
% Output:
%   A combined histogram is displayed showing the distribution of TapeStation 
%   predominant peak sizes with and without electronic ladder. The histogram 
...
%   plotTapeStationPredominantPeakHistogram('path/to/excelFile.xlsx')
    ...
    % Extract peak sizes
    peakSizes = dataTable{:, 5}; % Assuming peak sizes are in the fifth column (in bp).
    peakSizesEL = dataTable{:, 6}; % Assuming peak sizes with electronic ladder are in the sixth column (in bp).
    ...
    nexttile(tcl)
    h1 = histogram(peakSizes, 'FaceColor', 'b', 'FaceAlpha', 0.5, 'BinWidth',2, 'Normalization', 'probability');
    ...
    h2 = histogram(peakSizesEL, 'FaceColor', 'r', 'FaceAlpha', 0.5, 'BinWidth',2, 'Normalization', 'probability');
    % Update axis after plotting to adjust y-scale to percentage
    ax = gca; % Get current axis
    ax.XTick = 0:10:max(peakSizesEL);
    ax.YTick = 0:0.05:1; % Define Y-axis ticks from 0 to 1 (0% to 100%)
    ax.YTickLabel = arrayfun(@(x) sprintf('%.0f', x*100), ax.YTick, 'UniformOutput', false); % Convert to percentage labels

    % Add labels and title
    xlabel('dsDNA size of predominant TapeStation assay peak (bp)');
    ylabel('Percentage of Libraries (%)');
    title('Distribution of TapeStation Predominant Peak Sizes');
    ...
    svgFileName = 'TapeStationPredominantPeakSizeHistogram.svg';
